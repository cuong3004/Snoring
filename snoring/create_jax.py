...
class DynamicWrapper(nn.Module):
    ...
    # def setup(self):
    #     # Optionally, you can perform setup here.
...
class DY_Block(nn.Module):
    ...
    def setup(self):
        ...
        norm_layer = partial(nn.BatchNorm, not self.training, momentum=1-0.01, epsilon=0.001, axis=1)
        ...
        if self.cnf.expanded_channels != self.cnf.input_channels:
            ...
            self.exp_conv = DynamicWrapper(Identity())
            self.exp_norm = Identity()
            self.exp_act = DynamicWrapper(Identity())
